...
def train_batches_hypernet(train_loader,  train_data_num, lambda_sample_num,
                  lambda_encode, net, device, loss_fn, optimizer, label, rep = False):
    tq = tqdm.tqdm(train_loader)
    losses = []
    for x,y in tq:            
        x = x.to(device)
...
def test_acc_net(net, device, test_loader): # точность классификации
    correct = 0
    net.eval()
    cnt = 0
...
def test_acc_hyper(net, device, test_loader, lambda_encode, lambdas): # точность классификации
    net.eval()
    cnt = 0
    accs = {}
    for l in lambdas:
